...
export default class MainMenuScreen extends Component {
    ...
    return (
      <TouchableHighlight
        ...
        <ScrollView contentContainerStyle={styles.menuCard}>
          <Image source={require(item.image)} style={{ height: 80, width: 90 }} />
          <View style={styles.menu}>
            <Text style={styles.menuTitle}>{item.name}</Text>
            <Text style={styles.menuTitle}>RM {item.price}</Text>
          </View>
        </ScrollView>
  ...
  render() {
    return (
      <SafeAreaView style={{ flex: 1, backgroundColor: 'white' }}>
        {/* <View style={styles.header}></View> Forslider */}
        <FlatList
          showsHorizontalScrollIndicator={false}
          contentContainerStyle={{ paddingBottom: 80 }}
        ...
        {/* <FlatList
          ...
          renderItem={({ item }) => {
            ...
            <TouchableHighlight
              ...
              <ScrollView style={styles.menuCard}>
                <Image source={{ uri: URL + '/' + item.image}} style={{ height: 80, width: 90 }} />
                <View style={styles.menu}>
                  <Text style={styles.menuTitle}>{item.name}</Text>
